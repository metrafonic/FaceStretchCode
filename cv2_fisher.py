...
def prepare_training_data(data_folder_path):
    ...
    for dir_name in dirs:
        ...
        for image_name in subject_images_names:
            ...
            cv2.imshow("Training on image...", cv2.resize(image, (128, 128)))
            cv2.waitKey(100)
...
def main():
    trained_path = sys.argv[3]
    trained_base = os.path.dirname(trained_path)
    if not os.path.exists(trained_base):
        os.makedirs(trained_base)
    if not os.path.exists(trained_path):
        print("Preparing data...")
        faces, labels = prepare_training_data(sys.argv[1])
        print("Data prepared")
        dump((faces, labels), trained_path)
    else:
        faces, labels = load(trained_path)
    ...
    for person in next(os.walk(image_dir))[1]:
        ...
        for image_name in next(os.walk(persondir))[2]:
            ...
            face_data = cv2.imread(image_filepath)
            label, confidence = predict(face_data, face_recognizer)
            if not label:
                label = 0
                confidence = 0
            elif confidence != 0:
